...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contactForm) {
    ...
    if (providerGmail) {
      providerGmail.addEventListener('click', () => {
        ...
        const gmailUrl = `https://mail.google.com/mail/?view=cm&fs=1&to=aayushak0812@gmail.com&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(mailBody)}`;
    ...
    if (providerYahoo) {
      providerYahoo.addEventListener('click', () => {
        ...
        const { name, email, subject, message } = currentMailData;
        const mailBody = `Name: ${name}\nEmail: ${email}\n\nMessage:\n${message}`;
        const yahooUrl = `https://compose.mail.yahoo.com/?to=aayushak0812@gmail.com&subj=${encodeURIComponent(subject)}&body=${encodeURIComponent(mailBody)}`;
    ...
    if (providerDefault) {
      providerDefault.addEventListener('click', () => {
        ...
        const { name, email, subject, message } = currentMailData;
        const mailBody = `Name: ${name}\nEmail: ${email}\n\nMessage:\n${message}`;
        const mailtoUrl = `mailto:aayushak0812@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(mailBody)}`;
    ...
    if (btnSubmitWhatsapp) {
      btnSubmitWhatsapp.addEventListener('click', () => {
        ...
        setTimeout(() => {
          window.open(`https://wa.me/916290743570?text=${encodedText}`, '_blank');
          showFormFeedback('', '');
        }, 800);
      });
    }
  ...
  function sendFloatingWaMessage() {
    ...
    setTimeout(() => {
      const waUrl = `https://wa.me/916290743570?text=${encodeURIComponent(text)}`;
